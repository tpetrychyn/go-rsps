...
public class Region {
	...
	public static void load() {
		try {
			File f = new File("C:\\Users\\Taylor\\Desktop\\RSPS\\2006Redone Server\\data/world/map_index");
			byte[] buffer = new byte[(int) f.length()];
			DataInputStream dis = new DataInputStream(new FileInputStream(f));
			dis.readFully(buffer);
			dis.close();
			ByteStream in = new ByteStream(buffer);
			int size = 0;//in.length() / 7;
			...
			for (int i = 0; i < size; i++) {
				byte[] file1 = getBuffer(new File("C:\\Users\\Taylor\\Desktop\\RSPS\\2006Redone Server\\data/world/map/"
						+ mapObjectsFileIds[i] + ".gz"));
				byte[] file2 = getBuffer(new File("C:\\Users\\Taylor\\Desktop\\RSPS\\2006Redone Server\\data/world/map/"
